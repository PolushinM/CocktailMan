...
Как собираюсь реализовать:
1. Обучить Conditional Implicit Discriminator in Variational Autoencoder (C-IDVAE): на входе энкодера - изображение коктейля + "вектор рецепта" с ингридиентами, добавляется после первой свёртки, на выходе энкодера (после семплера) - латентный "вектор стиля" небольшой размерности (где-то 4-6) + (до семплера) выход классификатора (для GAN Loss), на входе генератора(декодера) - латентный "вектор стиля" из энкодера и "вектор рецепта" с ингридиентами. \
