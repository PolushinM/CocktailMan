...
[Интернет-сайт](https://cocktailman.herokuapp.com/), на котором работают три обученные мной нейросетевые модели:
1. Классификатор, который по фото коктейля подсказывает ингредиенты, из которых его можно приготовить. (Уже работает)
2. Детектор коктейля, который находит коктейль на фото и рисует рамку, а так же облегчает работу первой модели. (В разработке)
3. Генератор text2image, который принимает текс рецепта и рисует изображение коктейля. (В разработке)
